...
class GameArea{
    constructor(map, elevationMap, shadowMap, width, height){
                ...
                case "water":
                    if (this.map[x][y - 1][layout] === undefined) {
                        ...
                        if (this.map[x - 1][y][layout] === undefined || Math.floor((this.map[x - 1][y][layout] - 9000) / 8) === 2) {
                            setTimeout(() => {
                                if (this.map[x - 1][y][layout] === undefined || Math.floor((this.map[x - 1][y][layout] - 9000) / 8) === 2) this.placeBlock(x - 1, y, layout, this.makeFlowingWaterBlock(9000))
                            }, 200);
                        }
                        if (this.map[x + 1][y][layout] === undefined || Math.floor((this.map[x + 1][y][layout] - 9000) / 8) === 2) {
                            setTimeout(() => {
                                if (this.map[x + 1][y][layout] === undefined || Math.floor((this.map[x + 1][y][layout] - 9000) / 8) === 2) this.placeBlock(x + 1, y, layout, this.makeFlowingWaterBlock(9008))
                            }, 200);
                        }
                    }
                    break;
        ...
        this.destroyBlock = (x, y, layout) => {
            ...
            this.deleteLightRound(x, y, x, y, blockTable[lastBlock].brightness, blockTable[lastBlock].isNaturalLight === true);
        ...
        this.placeBlock = (x, y, layout, id) => {
            ...
            if (!this.map[x][y][layout] || (blockTable[this.map[x][y][layout]] && !blockTable[this.map[x][y][layout]].isSolid)) {
                ...
                this.addLightRound(x, y, x, y, blockTable[id].brightness, blockTable[id].isNaturalLight === true, false);
                this.updateRadius(x, y, layout);
                this.updateBlock(x, y, layout);
            }
        };
